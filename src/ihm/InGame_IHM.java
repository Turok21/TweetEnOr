...
public class InGame_IHM extends IHM_Iterface implements ActionListener, KeyListener, MouseListener {
	...
	protected Shared_component _shared;// objet d'échange avec CtrlTweetEnOr
										// pour la bar de progression du loader
	...
	public InGame_IHM(LEVEL difficulte, String hastag_theme, JFrame fram) throws IOException {
		...
		drawloader(0);// gestion de l'affichage du loader lors du chargement des
						// tweets
	...
	protected void load_game_data() {
		/***************
		 * chargement des données de jeu(twwets) dans un thread à pars pour ne
		 * pas freezer le loader
		 ***************/
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (e instanceof IllegalStateException) // credentials
															// missing
					{
	 ...
	 * 
	 * @param redraw
	 *            : si = 1 une nouvelle fram sera créé
	 * @param redraw
	 */
	protected void drawloader(int redraw) {
		...
		_fenetre.add(_Panel_loader);// ajoute le panel_loader et tout ses
									// composents à la fenetre
	...
	protected void draw_play_screen(int redraw) {
		...
		if (redraw == 1) {
			...
			_jp_principal = load_fenetre_and_panel_principale("Un Tweet en Or - Jeu ", "fond_Tweet_en_or.jpg",
					_fram_given, true);
		}
	...
	/**
	 * reraw vie Rafraichie les vies(oiseaux en haut a gauche)
	 */
		...
		if (mot_a_CTEO.isEmpty()) {
			...
			for (String mot : mots_a_CTEO) {
				...
				if (motVerifie.getPonderation() == -1) {
					...
					affichage += "Le mot " + motVerifie.getWord() + " est correct (" + motVerifie.getPonderation()
							+ " points) !";
	...
	/**
	 * loose_vie enleve une vie au joueur
	 */
	...
	/**
	 * add_point ajoute les point d'un mots a la somme de point total du joueur
	 * 
	 * @param nb_point
	 * @param mots
	 */
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (_b_hintShuffle.isSelected() && _nbAna > 0) {
	...
	@Override
	/**
	 * Keypressed gère les evenement des touche clavier préssée
	 */
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_DOWN) {
			...
			if (_maj >= 24) {
				for (Txt mot : _listword_label) {
					if (mot.getForeground().getRed() == 29 && mot.getForeground().getGreen() == 202
							&& mot.getForeground().getBlue() == 255)
		...
		if (e.getKeyCode() == KeyEvent.VK_UP) {
			...
			if (_tab >= 24) {
				for (Txt mot : _listword_label) {
					if (mot.getForeground().getRed() == 254 && mot.getForeground().getGreen() == 255
							&& mot.getForeground().getBlue() == 255)
